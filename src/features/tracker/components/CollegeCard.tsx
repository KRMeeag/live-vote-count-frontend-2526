...
const CollegeCard = ({ data, rank }: CollegeCardProps) => {
  ...
  const StandardLayout = () => (
    <div className="flex-grow flex flex-col justify-center h-full w-full">
      <div className="flex justify-between items-center mb-2">
        ...
        {!underReview && (
          <div
            className={`font-bold py-1.5 rounded-md text-xs md:text-sm flex items-center justify-center gap-1.5 w-[60px] md:w-[72px] flex-shrink-0 ${getRankStyle(rank)}`}
          >
            {rank <= 3 && (
              <Medal size={14} className="md:w-4 md:h-4" strokeWidth={2.5} />
            )}
            #{rank}
          </div>
        )}
      ...
      <div className="w-full block">
        <div
          className={`w-full h-2 md:h-3 rounded-full overflow-hidden ${underReview ? "bg-amber-100" : "bg-gray-200"}`}
        >
          {!underReview && (
            <div
  ...
  return (
    <div
      className={`relative bg-white shadow-sm border-l-8 ${theme.borderClass} ${isFirstPlace ? "p-4 md:p-6" : "p-4"} flex items-center w-full h-full gap-4 md:gap-5`}
    >
      <div
        className={`relative flex-shrink-0 ${logoLayout.container} transition-all duration-300`}
      >
        {theme.logos.length === 1 ? (
          <div className="absolute inset-0 flex items-center justify-center">
            <img
              src={theme.logos[0]}
              alt={`${college} logo`}
              className={`${logoLayout.single} object-contain drop-shadow-sm transition-all duration-300`}
            />
          </div>
        ) : (
          <div className="absolute inset-0 flex items-center justify-center">
        ...
        <>
          ...
          <div className="hidden md:flex flex-grow items-center justify-between h-full gap-6">
            <div className="flex-grow flex flex-col justify-center h-full">
              <h3 className="font-bold text-gray-900 text-2xl leading-tight mb-1">
                {college}
              </h3>
            ...
            <div className="flex-shrink-0 flex items-center h-full">
              <div
                className={`font-bold px-8 py-5 rounded-lg text-center flex flex-col items-center justify-center min-w-[220px] ${getRankStyle(1)}`}
              >
